...
alt.on('shownametag', drawNametags)

function drawNametags() {
let draw = alt.setInterval(()=>{
    ...
    for (let i = 0, n = alt.Player.all.length; i < n; i++) {
        ...
        const lineHeight = fontSize / drawDistance;
        ...
        // Bars
        if (showBarsOnAim && foundEntity && aimingAtPed === player.scriptID && !native.isEntityDead(player.scriptID,true)) {
            drawBarBackground(100, lineHeight, scale, 0.85, 139, 0, 0, 255);
            drawBar(native.getEntityHealth(player.scriptID) - 100, lineHeight, scale, 0.85, 255, 0, 0, 255);

            if (native.getPedArmour(player.scriptID) > 0) {
                drawBarBackground(100, lineHeight, scale, 1.10, 140, 140, 140, 255);
                drawBar(native.getPedArmour(player.scriptID), lineHeight, scale, 1.10, 255, 255, 255, 255);
            }
        }
