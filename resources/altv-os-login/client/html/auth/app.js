...
const app = new Vue({
    ...
    methods: {
        ...
        processRegistration(register = false) {
            ...
            if (register) {
                if (this.nickname === '') {
                    this.setError('Must specify a character name.');
                    return;
                }
    
                if (this.nickname.length <= 3) {
                    this.setError('Character name must be atleast 4 characters.');
                    return;
                }
            ...
            if ('alt' in window) {
                if (this.registering) {
                    alt.emit('auth:Try', this.username, this.password, this.email, this.nickname);
                    return;
                }
